...
static esp_err_t cmd_handler(httpd_req_t *req)
{
  ...
  buf_len = httpd_req_get_url_query_len(req) + 1;//从请求 URL 获取查询字符串长度。
  if (buf_len > 1) {//如果有响应
    ...
    //httpd_req_get_url_query_str :从请求 URL 获取查询字符串。
    //1).正在响应的请求;
    //2).缓冲区的指针（如果找到）;
    //3).输出缓冲区的长度
        if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {//缓冲区==>buf;缓冲区长度==>buf_len
          if (httpd_query_key_value(buf, "var", variable, sizeof(variable)) == ESP_OK &&  //获取 URL 查询标记var,val
...
{
    httpd_config_t config = HTTPD_DEFAULT_CONFIG();//结构体初始化
    ...
    httpd_uri_t cmd_uri = {//获取URL
        .uri       = "/control",//响应按键
        .method    = HTTP_GET,
        .handler   = cmd_handler,//回调函数
        .user_ctx  = NULL//交换对象
    };
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK) {//config–服务器新实例的配置
                                                        //handle–新创建的服务器实例的句柄。错误时为空
        httpd_register_uri_handler(camera_httpd, &index_uri);//httpd_register_uri_handler:URL处理程序通过传递类型结构的对象来注册
