...
static esp_err_t 相机处理(httpd_req_t *req)
{
  camera_fb_t *fb = NULL;
  esp_err_t res = ESP_OK;
  int64_t fr_start = esp_timer_get_time();//获取启动后的时间（以微秒为单位）Get time in microseconds since boot

  fb = esp_camera_fb_get();//获取指向帧缓冲区的指针。Obtain pointer to a frame buffer.
  if (!fb)
  {
  ...
  bool s;
  {
    ...
    else
    {
      ...
      httpd_resp_send_chunk(req, NULL, 0);//响应但不返回值
      fb_len = jchunk.len;
    }
  ...
  dl_matrix3du_t *矩阵 = dl_matrix3du_alloc(1, fb->width, fb->height, 3);
  //分配一个包含8位项目的3D矩阵，访问顺序为NHWC |N代表数量， C代表channel，H代表高度，W代表宽度. Allocate a 3D matrix with 8-bits items, the access sequence is NHWC
  if (!矩阵)
  {
    esp_camera_fb_return(fb);
  ...
  esp_camera_fb_return(fb);//返回要再次使用的帧缓冲区。Return the frame buffer to be reused again.
  if (!s)
  {
    dl_matrix3du_free(矩阵);
  ...
  dl_matrix3du_free(矩阵);//Free a matrix3d
  if (!s)
  {
...
static esp_err_t 视频流处理(httpd_req_t *req)
{
  camera_fb_t *fb = NULL;
  esp_err_t res = ESP_OK;
...
static esp_err_t 控制处理(httpd_req_t *req)
{
  char *buf;
  size_t buf_len;
  char variable[32] = {
  ...
  int val = atoi(value);//字符串转换为整数
  sensor_t *s = esp_camera_sensor_get();//获取指向图像传感器控制结构的指针Get a pointer to the image sensor control structure
  int res = 0;
  ...
  if (!strcmp(variable, "framesize"))
  {
    ...
    if (s->pixformat == PIXFORMAT_JPEG)
      res = s->set_framesize(s, (framesize_t)val);//设置视频大小
  }
  ...
  return httpd_resp_send(req, NULL, 0);//不对请求进行响应
}

static esp_err_t 状态处理(httpd_req_t *req)
{
  static char json_response[1024];

  sensor_t *指向图像传感器控制结构的指针 = esp_camera_sensor_get();//Get a pointer to the image sensor control structure
  //获取指向图像传感器控制结构的指针
  char *p = json_response;
  *p++ = '{';

  p += sprintf(p, "\"framesize\":%u,", 指向图像传感器控制结构的指针->status.framesize);
  p += sprintf(p, "\"quality\":%u,", 指向图像传感器控制结构的指针->status.quality);
  *p++ = '}';
  *p++ = 0;
    ...
    <br>
            ...
            <section id="buttons">
                ...
                <div id="controls" class="control-container">
                  <table>
                  <tr><td align="center"><button class="button button6" id="get-still">Image</button></td><td align="center"><button id="toggle-stream">视频流开始</button></td><td></td></tr>
                  <tr><td></td><td align="center"><button class="button button2" id="forward" onclick="fetch(document.location.origin+'/control?var=car&val=1');">前进</button></td><td></td></tr>
                  <tr><td align="center"><button class="button button2" id="turnleft" onclick="fetch(document.location.origin+'/control?var=car&val=2');">向左</button></td><td align="center"></td><td align="center"><button class="button button2" id="turnright" onclick="fetch(document.location.origin+'/control?var=car&val=4');">向右</button></td></tr>
                  <tr><td></td><td align="center"><button class="button button2" id="backward" onclick="fetch(document.location.origin+'/control?var=car&val=5');">后台</button></td><td></td></tr>
                  <tr><td></td><td align="center"><button class="button button4" id="flash" onclick="fetch(document.location.origin+'/control?var=flash&val=1');">开灯</button></td><td></td></tr>
                  <tr><td></td><td align="center"><button class="button button4" id="flashoff" onclick="fetch(document.location.origin+'/control?var=flashoff&val=0');">关灯</button></td><td></td></tr>
                  </table>
                </div>
                <br>
        ...
        <script>
          ...
          break;case'range':case'select-one':C=B.value;
          break;
          ...
          fetch(D).then
          (E=>{console.log(`request to ${D} finished, status: ${E.status}`)})
          }
...
{
  httpd_config_t HTTP设置 = HTTPD_DEFAULT_CONFIG(); //HTTP默认配置==================================================第零步：设置置基本的响应请求(回调函数)

  httpd_uri_t 索引url = {//设置响应内容类型，并上传HTML文件
                           .uri = "/",
                           .method = HTTP_GET,
                           .handler = 索引处理, //设置响应内容类型，并上传HTML文件
                           .user_ctx = NULL};

  httpd_uri_t 状态url = {//设置json回应
                            .uri = "/status",
                            .method = HTTP_GET,
                            .handler = 状态处理,
                            .user_ctx = NULL};

  httpd_uri_t 控制url = {//获取URL信息
                         .uri = "/control",
                         .method = HTTP_GET,
                         .handler = 控制处理,
                         .user_ctx = NULL};

  httpd_uri_t 相机url = {//处理相机请求
                             .uri = "/capture",
                             .method = HTTP_GET,
                             .handler = 相机处理,
                             .user_ctx = NULL};

  httpd_uri_t 视频流url = {//处理视频流请求
                            .uri = "/stream",
                            .method = HTTP_GET,
                            .handler = 视频流处理,
                            .user_ctx = NULL};

  Serial.printf("在端口上启动web服务器: '%d'\n", HTTP设置.server_port);
  if (httpd_start(&camera_httpd, &HTTP设置) == ESP_OK)
  { //=============================================第一步：httpd_start() 配置,开始配置
    httpd_register_uri_handler(camera_httpd, &索引url);
    httpd_register_uri_handler(camera_httpd, &控制url);
    httpd_register_uri_handler(camera_httpd, &状态url);
    httpd_register_uri_handler(camera_httpd, &相机url);
  }

  HTTP设置.server_port += 1;//80+1
  HTTP设置.ctrl_port += 1;//32768+1
  Serial.printf("Starting stream server on port: '%d'\n", HTTP设置.server_port);
  if (httpd_start(&stream_httpd, &HTTP设置) == ESP_OK)
  {
    httpd_register_uri_handler(stream_httpd, &视频流url);
  }
}
