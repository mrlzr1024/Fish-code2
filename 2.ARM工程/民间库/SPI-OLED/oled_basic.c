...
{
	OLED_ClearScreenBuffer(0);
}
//更新屏幕 注意此处特指刷新屏幕数组而不是临时数组
void OLED_UpdateScreen(void)
{
	OLED_UpdateScreenDisplay();
}
...
void FillRect(int x,int y,int width,int height)
{
	...
	if(x>SCREEN_COLUMN ||y>SCREEN_ROW)   
			return;
	for(i=x; i<x+width&&i<SCREEN_COLUMN; i++)
	{
		if( temp==0 )
		{
...
//画一条起点坐标为x,y 长度为height的竖线 (与OLED_FastHLine()类似 不过后者是使用打点实现)
...
unsigned char FrameRateOLED_UpdateScreen(int value)
{
	if(OledTimeMs==0)
	{
		OLED_UpdateScreen();
