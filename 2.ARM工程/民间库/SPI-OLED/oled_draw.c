...
{
	OLED_Line ( left, top, right, top );
	OLED_Line ( left, bottom , right , bottom );
	OLED_Line ( left, top, left, bottom );
	OLED_Line ( right , top, right , bottom );	
}
//功能:使用对角点填充矩形
void OLED_FillRect1(int left,int top,int right,int bottom)
{
	OLED_Rect1(left,top,right,bottom);
	FillRect(left+1,top+1,right-left-1,bottom-top-1);	
}
//左上角坐标 矩形宽高
void OLED_Rect2(int left,int top,int width,int height)
{
		OLED_Line ( left, top, left+width-1, top );
		OLED_Line ( left, top+height-1 , left+width-1 , top+height-1 );
		OLED_Line ( left, top, left, top+height-1);
		OLED_Line ( left+width-1 , top, left+width-1 , top+height-1);	
}
//填充矩形
void OLED_FillRect2(int left,int top,int width,int height)
{
	//先用上面的函数画外框
	OLED_Rect2(left,top,width,height);
	//然后填充实心
...
//填充圆
void OLED_FillCircle( int usX_Center, int usY_Center, int r)
{
	OLED_FastVLine(usX_Center, usY_Center-r, 2*r+1);
	OLED_FillCircleHelper(usX_Center,usY_Center, r, 3, 0);
}
...
void OLED_FillCircleHelper(int x0, int y0, unsigned char r, unsigned char cornername, int delta)
{
	...
	while (x < y)
	{
		...
		{
			OLED_FastVLine(x0+x, y0-y, 2*y+1+delta);
			OLED_FastVLine(x0+y, y0-x, 2*x+1+delta);
		}
...
void OLED_RoundRect(int x, int y, unsigned char w, unsigned char h, unsigned char r)
{
	...
	OLED_FastHLine(x+r, y, w-2*r); 		// Top
	OLED_FastHLine(x+r, y+h-1, w-2*r); 	// Bottom
	OLED_FastVLine(x, y+r, h-2*r); 		// Left
	OLED_FastVLine(x+w-1, y+r, h-2*r); 	// Right
	//画出四个圆角
	OLED_CircleHelper(x+r, y+r, r, 1);
	OLED_CircleHelper(x+w-r-1, y+r, r, 2);
	OLED_CircleHelper(x+w-r-1, y+h-r-1, r, 4);
	OLED_CircleHelper(x+r, y+h-r-1, r, 8);
}
//画实心圆角矩形
void OLED_FillRoundRect(int x, int y, unsigned char w, unsigned char h, unsigned char r)
{
  //画实心矩形
  OLED_FillRect2(x+r, y, w-2*r, h);

  //再填充左右两边
  OLED_FillCircleHelper(x+w-r-1, y+r, r, 1, h-2*r-1);	//右上角的圆心
  OLED_FillCircleHelper(x+r, y+r, r, 2, h-2*r-1);		//左下角的圆心
}

/////////////////////////////////////////////////////////////////////////////////////////////////////
//画椭圆
//圆心 2个轴长
void OLED_Ellipse(int xCenter,int yCenter,int Rx,int Ry)
{
	int Rx2=Rx*Rx;
	int Ry2=Ry*Ry;
	int twoRx2=2*Rx2;
...
//填充一个椭圆 参数同上
void OLED_FillEllipse(int x0, int y0,int rx,int ry)
{
	int x, y;
	int xchg, ychg;
	int err;
	...
	while( stopx >= stopy )
	{
		OLED_FastVLine( x0+x, y0-y, y+1);
		OLED_FastVLine( x0-x, y0-y, y+1);
		OLED_FastVLine( x0+x, y0, y+1);
		OLED_FastVLine( x0-x, y0, y+1);
		y++;
		stopy += rxrx2;
		err += ychg;
...
{
	//很简单  就是画3条任意线
	OLED_Line(x0, y0, x1, y1);
	OLED_Line(x1, y1, x2, y2);
	OLED_Line(x2, y2, x0, y0);
}
//填充三角形
void OLED_FillTriangle(int x0, int y0, int x1, int y1, int x2, int y2)
{
	...
	OLED_Set_DrawColor(GetFillColor());
	if (y0 > y1)
	{
...
{
	unsigned char i=0,j=0,type = 0;
	unsigned char fontbuf[32];
	while(text[i]>0x00)
	{

		type = OLED_Get_Type(text[i], text[i + 1]);
		if((y > SCREEN_COLUMN - 16 + 8 * (type == 3)) || (text[i] == 0x0A))
		{
			y = 0;
			x += 2;
		}
		switch(type)
		{
			case 0:
				...
				break;
			case 1:
				OLED_Get_front(text[i], text[i + 1], fontbuf, type);		
				for(j=0;j<16;j++)
					WriteByteBuffer(x,y+j,fontbuf[j]);
				for(j=0;j<16;j++)
					WriteByteBuffer(x+1,y+j,fontbuf[j+16]);
			...
			case 3:
				...
				for(j=0;j<8;j++)
					WriteByteBuffer(x,y+j,fontbuf[j]);
				for(j=0;j<8;j++)
					WriteByteBuffer(x+1,y+j,fontbuf[j+8]);
				y += 8;
				i += 1;
				break;
		}
...
{
	unsigned char Buf_Len = SCREEN_PAGE_NUM * SCREEN_PAGEDATA_NUM / 8;
	unsigned char Printf_buf[Buf_Len];
	memset(Printf_buf, 0,Buf_Len);
	va_list args;
	va_start(args, fmt);
	vsnprintf((char *)Printf_buf, Buf_Len, (char *)fmt,args);
	va_end(args);
	OLED_GBString(x, y, Printf_buf);
	if(Printf_Flag == PRINT_NOW)	OLED_UpdateScreen();
}
